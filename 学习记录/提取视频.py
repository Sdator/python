...
async def fetch(**post):
    # 利用 run_in_executor 把普通函数转为异步函数
    future = asyncio.get_event_loop().run_in_executor(
        None, partial(requests.post, **post))
    return await future
...
batchId=%s''' % (self.课程ID, self.时间戳),
        ...
        jsCode = res.text
        # 排除一些无法运行的代码
        jsCode = jsCode[jsCode.find('var'):jsCode.rfind(
            'dwr.engine._remoteHandleCallback')]
        # 用函数包装 js 代码
        abc = execjs.compile('function abc(){'+jsCode+';return s3}')
        data = abc.call('abc')
        print(2222, data)
        return res

    async def 获取视频地址(self, 视频ID):
        post = {
...
c0-param0=string:%s
c0-param1=string:%s
batchId=%s''' % (视频ID, self.课程ID, self.时间戳),
            'head': {
                "providerid": self.作者ID,
                "content-type": "text/plain"
            }}
...
async def main():
    info = {'课程ID': '1512007', '作者ID': '9660634'}
    a = A(**info)
    课程 = await a.获取课程()
    print(22222)
